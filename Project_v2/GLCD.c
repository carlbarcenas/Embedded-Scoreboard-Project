...
void LCD_draw_cube(int x0, int y0, uint16_t color){
	LCD_SetPoint(x0, y0, color);
	LCD_SetPoint(x0+1, y0, color);
	LCD_SetPoint(x0-1, y0, color);
	LCD_SetPoint(x0, y0+1, color);
	LCD_SetPoint(x0, y0-1, color);
	LCD_SetPoint(x0+1, y0+1, color);
	LCD_SetPoint(x0+1, y0-1, color);
	LCD_SetPoint(x0-1, y0+1, color);
	LCD_SetPoint(x0-1, y0-1, color);
}

void LCD_draw_teamholders(){
	int y0 = 0;
	int x0 = 0;
	int x1 = 130;
	int x2 = 190;
	int x3 = 320;
	for(y0 = 120; y0 < 150; y0++){
		LCD_DrawLine(x0,y0,x1,y0,Black);
		LCD_DrawLine(x2,y0,x3,y0,Black);
	}
}

void LCD_draw_pointholders(){
	int y0 = 0;
	int x0 = 0;
	int x1 = 70;
	int x2 = 250;
	int x3 = 320;
	for(y0 = 170; y0 < 200; y0++){
		LCD_DrawLine(x0,y0,x1,y0,Black);
		LCD_DrawLine(x2,y0,x3,y0,Black);
	}
}

void LCD_draw_home_side(uint16_t color, uint16_t secondary){
	int y0 = 0;
	int x0 = 0;
	int x1;
	for(y0 = 0; y0 < 240; y0++){
	...
	LCD_draw_basketball(65,60, secondary);
}

void LCD_draw_opponent_side(uint16_t color, uint16_t secondary, char * team1, char * team2, int points1, int points2){
    int y0 = 0;
    int x0 = 320;
    int x1;
    char text[20];
    for(y0 = 0; y0 < 240; y0++){
        x1 = 150 + ((40*(y0))/240);
        LCD_DrawLine(x1,y0,x0,y0,color);
        LCD_SetPoint(x1, y0, Black);
    }
    LCD_draw_pointholders();
    LCD_draw_teamholders();

    sprintf(text,"%d",points1);
    LCD_PutText(5,130,(unsigned char *)team1,White,Black);
    LCD_PutText(40,180, (unsigned char *)text, White, Black);

    sprintf(text,"%d",points2);
    LCD_PutText(192,130,(unsigned char *)team2,White,Black);
    LCD_PutText(255,180, (unsigned char *)text, White, Black);


    LCD_draw_basketball(255,60, secondary);
}

void LCD_draw_basketball(int x0, int y0, uint16_t color){
	int i = 0;
	
	LCD_draw_circle(x0,y0, 34, color);
	LCD_draw_circle(x0,y0, 35, color);
	LCD_draw_cube(x0, y0, color);
	
	for(i = 1; i < 12; i++){
		LCD_draw_cube(x0, y0+(3*i), color);
		LCD_draw_cube(x0, y0-(3*i), color);
		LCD_draw_cube(x0+(3*i), y0, color);
		LCD_draw_cube(x0-(3*i), y0, color);
	}
	
	for(i = 1; i<4; i++){
		LCD_draw_cube(x0-15, y0+(3*i), color);
		LCD_draw_cube(x0-15, y0-(3*i), color);
		LCD_draw_cube(x0+15, y0+(3*i), color);
		LCD_draw_cube(x0+15, y0-(3*i), color);
	}
	
	LCD_draw_cube(x0-18, y0+(12), color);
	LCD_draw_cube(x0-18, y0+(15), color);
	LCD_draw_cube(x0-18, y0-(12), color);
	LCD_draw_cube(x0-18, y0-(15), color);
	LCD_draw_cube(x0+18, y0+(12), color);
	LCD_draw_cube(x0+18, y0+(15), color);
	...
	LCD_draw_cube(x0-21, y0+(18), color);
	LCD_draw_cube(x0-21, y0-(18), color);
	LCD_draw_cube(x0+21, y0+(18), color);
	LCD_draw_cube(x0+21, y0-(18), color);
	
	LCD_draw_cube(x0-24, y0+(21), color);
	LCD_draw_cube(x0-24, y0-(21), color);
	LCD_draw_cube(x0+24, y0+(21), color);
	LCD_draw_cube(x0+24, y0-(21), color);
